...
| Refresh and logout use automatically attached cookies | TB-02, TB-03 | `httpOnly`, `Secure` in production, path scoping, origin-restricted CORS, and a double-submit `x-csrf-token`/`ft_csrf_token` cookie check (`GET /api/auth/csrf-token`) | Cross-site request protections rely on cookie policy, CORS, and the CSRF token check; changes to `SameSite`, origins, or cookie/CSRF endpoints need focused review |
